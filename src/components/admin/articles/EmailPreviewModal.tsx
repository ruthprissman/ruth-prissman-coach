...
const EmailPreviewModal: React.FC<EmailPreviewModalProps> = ({
  ...
  const generateEmailPreview = () => {
    ...
    return `
      <div dir="rtl" style="direction: rtl; font-family: 'Heebo', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: transparent; border-radius: 8px; overflow: hidden;">
        ...
        <div style="padding: 20px; background-color: transparent; margin: 0 20px 20px; text-align: center;">
  ...
  const generateLinksPreview = (links: Array<{id: number, fixed_text: string, url: string}> = []) => {
    ...
    const linksHtml = links.map(link => {
      ...
      let linkHtml = '<p style="font-family: \'Alef\', Arial, sans-serif; font-size: 14px; text-align: center; margin-bottom: 20px; color: #4A148C; font-weight: bold; text-shadow: 1px 1px 3px rgba(255, 255, 255, 0.7); direction: rtl;">';
      ...
      if (link.url && link.url.startsWith('https://wa.me/')) {
        ...
        linkHtml += `<a href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer" 
                      ...
                      </svg>${escapeHtml(link.fixed_text)}</a>`;
      } 
      // Regular link with URL
        ...
        linkHtml += `<a href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer" 
                    ...
                    ${escapeHtml(link.fixed_text)}</a>`;
      } 
      // Text only (no URL)
      else {
        console.log('[EmailPreviewModal] Creating text-only item (no URL)');
        linkHtml += `<strong style="color: #4A148C; text-shadow: 1px 1px 3px rgba(255, 255, 255, 0.7); font-family: 'Alef', Arial, sans-serif;">
                    ${escapeHtml(link.fixed_text)}</strong>`;
      }
  ...
  const formatContent = (markdown: string) => {
    ...
    // Replace links with Alef font styling
    formatted = formatted.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" style="color: #4A148C; font-weight: bold; text-decoration: none; text-shadow: 1px 1px 3px rgba(255, 255, 255, 0.7); font-family: \'Alef\', Arial, sans-serif; text-align: right; direction: rtl;">$1</a>');

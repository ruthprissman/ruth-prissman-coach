...
const EmailPreviewModal: React.FC<EmailPreviewModalProps> = ({
  ...
  const generateEmailPreview = () => {
    ...
    return `
      <div dir="rtl" style="direction: rtl; font-family: 'Heebo', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: rgba(255, 255, 255, 0.85); border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); overflow: hidden;">
        ...
        <div style="padding: 20px; background-color: rgba(255, 255, 255, 0.5); margin: 0 20px 20px; border-radius: 4px; text-align: right;">
          <h3 style="color: #4A148C; text-shadow: 1px 1px 3px rgba(255, 255, 255, 0.7); margin-top: 0; font-family: 'Alef', sans-serif; text-align: right;">קישורים מומלצים:</h3>
  ...
  const generateLinksPreview = (links: Array<{id: number, title: string, url: string}> = []) => {
    ...
    return links.map(link => {
      ...
      // Text only (no URL)
        ...
        linkHtml += `<strong style="color: #4A148C; text-shadow: 1px 1px 3px rgba(255, 255, 255, 0.7);">
  ...
  const formatContent = (markdown: string) => {
    ...
    formatted = formatted.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" style="color: #4A148C; font-weight: bold; text-decoration: none; text-shadow: 1px 1px 3px rgba(255, 255, 255, 0.7); font-family: \'Alef\', sans-serif; text-align: right; direction: rtl;">$1</a>');
